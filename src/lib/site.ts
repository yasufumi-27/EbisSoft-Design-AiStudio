...
export const siteConfig = {
  ...
  /**
   * メタディスクリプション（`<meta name="description">`・OG・Twitter に出る本文）。
   *
   * 日本語の検索結果でスニペットとして表示されるのは **おおむね全角80〜120字** で、
   * それを超えた分は「…」で切り捨てられる（＝読まれない）。以前ここには190字の
   * 文章が入っていたが、後半（組み込み・LLMO・京都商工会議所）は表示されていなかった。
   * クリック判断に効く「誰が・どこで・何を」だけをこの長さに収め、
   * 網羅的な説明は下の longDescription（構造化データ・llms.txt 用）に置く。
   */
  description:
    "京都市伏見区のエビスソフトは、生成AIを制作フロー全体に組み込むWeb制作事業者です。AIチャットボット・3DCG演出・システム連携から組み込みソフトウェア開発まで一貫対応。SEO・AEO・LLMO対策込みで、初回相談は無料です。",
  /**
   * 網羅版の説明。**人が読むスニペットではなく、機械が読む場所**に使う。
   * 構造化データ（Organization / WebSite の description）と llms.txt が対象で、
   * ここは長さ制限がないぶん、事業領域を取りこぼさずに書く（LLMO）。
   */
  longDescription:
  ...
  contact: {
    ...
    /**
     * 地図のURL（構造化データ hasMap）。
     * 座標だけだと Google 側で所在地の照合先が定まらないため、地図サービス上の
     * 同じ地点を明示しておく。ローカル検索（「京都市伏見区 ホームページ制作」等）で
     * 事業所の実在性を裏づけるシグナルになる。
     */
    mapUrl: "https://www.google.com/maps/search/?api=1&query=34.949037,135.724274",
  },
