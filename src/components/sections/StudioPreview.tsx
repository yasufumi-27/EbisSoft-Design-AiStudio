  ...
  return (
    <div className={styles.experience} data-enter>
      <div className={styles.choices} role="group" aria-label="活用シーンを選ぶ">
        {examples.map((example, index) => <button key={example.tag} type="button" aria-pressed={selected === index} aria-controls="studio-example" onClick={() => setSelected(index)}><Icon name={example.icon}/>{example.label}</button>)}
      </div>
      <div id="studio-example" className={styles.example} data-tilt="soft">
        ...
        <div key={selected} className={`${styles.exampleVisual} ${styles[`scene${selected}`]}`}>
          ...
          {selected === 0 ? <div className={styles.chatWindow}>
            ...
            <div className={styles.answer}><span className={styles.answerIcon}><Icon name="sparkles"/></span><div><b>マニュアルから、必要な手順を。</b><p>1. 電源とネットワークを接続<br/>2. 管理画面で機器を登録<br/>3. 接続状態を確認して、設定完了</p><span className={styles.source}><Icon name="check"/> 参照：製品マニュアル p.12</span></div></div>
            ...
            <div className={styles.productBottom}><div><b>かたちも、質感も。</b><span>色や角度を変えて確認</span></div><div className={styles.swatches} aria-hidden="true"><i/><i/><i/></div></div>
          </div> : <div className={styles.systemWindow}>
            <div className={styles.windowBar}><span><Icon name="gauge"/> 設備モニター</span><span className={styles.statusDot}>サンプル</span></div>
            <div className={styles.sensorStats}><div><span>室温</span><b>24.8<small> °C</small></b></div><div><span>湿度</span><b>46<small> %</small></b></div><div><span>接続機器</span><b>08<small> 台</small></b></div></div>
            ...
            <div className={styles.connection}><span>機器</span><i/><span>クラウド</span><i/><span>Web</span></div>
          </div>}
        </div>
      </div>
    </div>
