  ...
  if (sent) {
    return (
      <div className="ds-form-done" role="status">
        <b>{jaNode("送信されました（デモのため、実際には送信していません）。")}</b>
        <br />
  ...
  return (
    ...
    >
      <div className="ds-field">
        <label htmlFor="ds-name">{jaNode("お名前")}</label>
        <input id="ds-name" name="name" autoComplete="name" required />
      </div>
      <div className="ds-field">
        <label htmlFor="ds-email">{jaNode("メールアドレス")}</label>
        <input id="ds-email" name="email" type="email" autoComplete="email" required />
      </div>
      <div className="ds-field">
        <label htmlFor="ds-subject">{jaNode("ご用件")}</label>
      ...
      <div className="ds-field">
        <label htmlFor="ds-body">{jaNode("お問い合わせ内容")}</label>
        <textarea id="ds-body" name="body" rows={5} required />
      </div>
      <p className="ds-form-note">{jaNode("これはデモサイトのフォームです。入力された内容はどこにも送信・保存されません。")}</p>
      <div>
        <button type="submit" className="ds-btn ds-btn--primary">{jaNode("送信する")}</button>
      </div>
    </form>
  );
}
