      ...
      <div className="ds-bar">
        <div className="ds-wrap ds-bar-inner">
          <details className="ds-disclosure">
            <summary><strong>{jaNode("デモサイト")}</strong><span>{jaNode("事業者・掲載情報は架空です")}</span><small>{jaNode("詳細")}</small></summary>
            <div className="ds-disclosure-body">
              <p>{jaNode(site.brand)}{jaNode("は架空の事業者で、住所・電話番号・お客様の声もすべて架空です。")}{jaNode(siteConfig.name)}{jaNode("が「")}{jaNode(data.name)}{jaNode("のホームページを作るとこうなる」を見せるために制作しました。")}</p>
              <a className="ds-bar-design" href={`https://yasufumi-27.github.io/EbisSoft-Design-Proposals/proposal/${proposal.id}/`} target="_blank" rel="noopener">
                DESIGN {jaNode(proposal.no)} / {jaNode(proposal.name)}<i>{proposal.jp}</i>
              </a>
            </div>
          </details>
          <a className="ds-bar-link" href={`${siteConfig.url}/showcase/${data.slug}`}>{jaNode("構成を見る →")}</a>
        </div>
      </div>
      ...
      <header className="ds-header">
        <div className="ds-wrap ds-header-inner">
          <a href="#top" className="ds-brand">
            <b>{jaNode(site.brand)}</b>
            <span>{jaNode(site.brandEn)}</span>
          </a>
          ...
          <details className="ds-menu">
            ...
            <div className="ds-menu-panel">
              <div className="ds-wrap">
                ...
                <a href="#contact">{jaNode("お問い合わせ")}</a>
              </div>
            </div>
          </details>
        </div>
        ...
        <section className="ds-hero" id="top">
          <div className="ds-wrap ds-hero-grid">
            <div className="ds-hero-copy">
              <span className="ds-hero-kicker">{jaNode(data.name)} WEBSITE CONCEPT</span>
              <h1>
                <span className="ds-title-line">{jaNode(site.hero[0])}</span>
                <span className="ds-title-line">{jaNode(site.hero[1])}</span>
              </h1>
              <p className="ds-hero-lead">{jaNode(site.lead)}</p>
              <div className="ds-hero-actions">
                <a href="#contact" className="ds-btn ds-btn--primary">
                  {jaNode(site.cta.primary)}
                </a>
                <a href="#menu" className="ds-btn ds-btn--ghost">
                  {jaNode(site.cta.secondary)}
                </a>
              </div>
              <ul className="ds-points">
                {site.heroPoints.map((p) => (
                  <li key={p}>{jaNode(p)}</li>
                ))}
              </ul>
            </div>
            <div className="ds-photo ds-hero-visual">
              <strong className="ds-visual-mark">{jaNode(site.brandEn)}</strong>
              ...
              <span>{jaNode("写真が入ります（本番ではお客様の素材に差し替え）")}</span>
            </div>
          </div>
        </section>
            ...
            <ul className="ds-news-list">
              {site.news.map((n) => (
                <li key={n.date}>
                  ...
                  <span className="ds-tag">{jaNode(n.tag)}</span>
                  <span style={{ minWidth: 0 }}>{jaNode(n.text)}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <section id="about" className="ds-section ds-block-about">
          <div className="ds-wrap">
            <div className="ds-head ds-head--center">
              ...
              <h2 className="ds-h2">{jaNode("選ばれている理由")}</h2>
              <p className="ds-lead">{jaNode(data.customer)}{jaNode("のために、次の3点を大切にしています。")}</p>
            </div>
            <div className="ds-grid ds-grid--3">
              {site.reasons.map((r, i) => (
                <div key={r.title} className="ds-card">
                  <span className="ds-num">{String(i + 1).padStart(2, "0")}</span>
                  <h3>{jaNode(r.title)}</h3>
                  <p>{jaNode(r.body)}</p>
                </div>
              ))}
            </div>
        ...
        <section id="menu" className="ds-section ds-section--alt ds-block-menu">
          <div className="ds-wrap">
            <div className="ds-head">
              ...
              <h2 className="ds-h2">{jaNode(site.menuTitle)}</h2>
              <p className="ds-lead">{jaNode(site.menuLead)}</p>
            </div>
            <div className="ds-menu-list">
              {data.catalog.map((c) => (
                <div key={c.sku} className="ds-menu-row">
                  <b>{jaNode(c.name)}</b>
                  <span className="ds-cat">{jaNode(c.category)}</span>
                  <span className="ds-price">
                    {jaNode(yen(c.price))}
                    <small>{jaNode(c.stock === 0 ? "／受付終了" : `／残り${c.stock}`)}</small>
                  </span>
                </div>
              ))}
            </div>
            <p className="ds-lead" style={{ marginTop: "1.25rem" }}>{jaNode("在庫・残数は")}{jaNode(data.systems[0]?.label ?? "基幹システム")}{jaNode("と連動する想定です。")}</p>
          </div>
        </section>
        ...
        <section id="feature" className="ds-section ds-block-feature">
          <div className="ds-wrap">
            <div className="ds-head">
              ...
              <h2 className="ds-h2">{jaNode("このサイトでできること")}</h2>
              <p className="ds-lead">{jaNode(site.featureLead)}</p>
            </div>
        ...
        <section className="ds-section ds-block-voices">
          <div className="ds-wrap">
            <div className="ds-head ds-head--center">
              ...
              <h2 className="ds-h2">{jaNode("お客様の声")}</h2>
              <p className="ds-lead">{jaNode("掲載している声は、デモ用に作成した架空のものです。")}</p>
            </div>
            <div className="ds-grid ds-grid--3">
              {site.voices.map((v) => (
        ...
        <section className="ds-section ds-section--alt ds-block-flow">
          <div className="ds-wrap">
            <div className="ds-head">
              ...
              <h2 className="ds-h2">{jaNode("ご利用の流れ")}</h2>
            </div>
            <ol className="ds-flow">
              {site.flow.map((f) => (
                <li key={f.title}>
                  <h3>{jaNode(f.title)}</h3>
                  <p>{jaNode(f.body)}</p>
                </li>
              ))}
            </ol>
        ...
        <section id="faq" className="ds-section ds-block-faq">
          <div className="ds-wrap">
            <div className="ds-head">
              ...
              <h2 className="ds-h2">{jaNode("よくある質問")}</h2>
            </div>
            <div className="ds-faq">
              {site.faq.map((f, i) => (
                <details key={f.q} open={i === 0}>
                  <summary>{jaNode(f.q)}</summary>
                  <p>{jaNode(f.a)}</p>
                </details>
              ))}
            </div>
        ...
        <section id="access" className="ds-section ds-section--alt ds-block-access">
          <div className="ds-wrap">
            <div className="ds-head">
              ...
              <h2 className="ds-h2">{jaNode("アクセス・営業情報")}</h2>
            </div>
            <div className="ds-grid ds-grid--2">
              <table className="ds-info">
                <tbody>
                  <tr>
                    <th>所在地</th>
                    <td>{jaNode(site.info.address)}</td>
                  </tr>
                  <tr>
                    <th>アクセス</th>
                    <td>{jaNode(site.info.access)}</td>
                  </tr>
                  <tr>
                    <th>営業時間</th>
                    <td>{jaNode(site.info.hours)}</td>
                  </tr>
                  <tr>
                    <th>定休日</th>
                    <td>{jaNode(site.info.closed)}</td>
                  </tr>
                  <tr>
                    <th>そのほか</th>
                    <td>{jaNode(site.info.extra)}</td>
                  </tr>
                </tbody>
              </table>
              <div className="ds-photo ds-photo--wide">
                <span>{jaNode("地図が入ります（本番ではGoogleマップを埋め込み）")}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="contact" className="ds-section ds-block-contact">
          <div className="ds-wrap">
            <div className="ds-head">
              ...
              <h2 className="ds-h2">{jaNode("お問い合わせ")}</h2>
              <p className="ds-lead">{jaNode("お電話（")}{jaNode(site.tel)}／{jaNode(site.info.hours)}{jaNode("）でも承ります。")}</p>
            </div>
            <div className="ds-grid ds-grid--2">
              <DemoSiteForm
              ...
              <div className="ds-card">
                <h3>{jaNode("この構成をご自身の事業で使うには")}</h3>
                <p>{jaNode("このページは")}{jaNode(siteConfig.name)}{jaNode("が制作したデモサイトです。文章・写真・メニューを お客様の内容に差し替えれば、そのまま公開できる状態まで作り込んでいます。 上の「このサイトでできること」に並んでいる機能も、すべて実装済みのものです。")}</p>
                <p style={{ marginTop: "1rem" }}>
                  <a
                    ...
                    rel="noopener"
                  >
                    {jaNode(siteConfig.name)}{jaNode("に相談する")}</a>
                </p>
              </div>
            </div>
          </div>
      ...
      <footer className="ds-footer">
        <div className="ds-wrap">
          <div className="ds-footer-grid">
            <div>
              <h2>{jaNode(site.brand)}</h2>
              <p style={{ marginTop: "0.6rem" }}>{jaNode(site.brandNote)}</p>
              <p style={{ marginTop: "0.6rem" }}>{jaNode(site.info.address)}</p>
              <p>TEL {jaNode(site.tel)}</p>
            </div>
            <div>
              <h2>{jaNode("ページ")}</h2>
            ...
            <div>
              <h2>{jaNode("営業時間")}</h2>
              <p style={{ marginTop: "0.6rem" }}>{jaNode(site.info.hours)}</p>
              <p>{jaNode("定休日：")}{jaNode(site.info.closed)}</p>
              <p style={{ marginTop: "0.6rem" }}>{jaNode(site.info.extra)}</p>
            </div>
          </div>

          <p className="ds-footer-note">{jaNode("※ このサイトは")}{jaNode(siteConfig.name)}{jaNode("（京都市伏見区）が制作した")}<b>{jaNode("デモサイト")}</b>{jaNode("です。")}{jaNode(site.brand)}{jaNode("は架空の事業者であり、掲載している住所・電話番号・お客様の声・実績は すべて架空のものです。実在の企業・団体とは関係ありません。")}<br />
            <Link href="/" prefetch={false}>
              {jaNode(siteConfig.name)}{jaNode("のサイトへ")}</Link>
            　／
            <Link href={`/showcase/${data.slug}`} prefetch={false}>
              {jaNode(data.name)}{jaNode("向けの機能の説明を見る")}</Link>
          </p>
        </div>
      </footer>
    </div>
